...
### Codex-/GitHub-Synchronisation

- Alte Codex-Arbeitsbereiche können auf einem Snapshot basieren und sind nicht automatisch garantiert auf dem aktuellen GitHub-`main`.
- Wenn `git fetch origin main` im Codex-Container fehlschlägt oder kein `origin/main` vorhanden ist, muss der aktuelle GitHub-Stand explizit über Browser/Raw-Datei geprüft und die Abweichung benannt werden.
- Für Code-Reparaturen nach einem kaputten GitHub-Web-Merge immer einen neuen Codex-Arbeitsbereich direkt vom aktuellen Repository-`main` starten oder vor Änderungen erfolgreich `git fetch origin main`/`git rebase origin/main` ausführen.
- PRs mit Konflikten nicht über den GitHub-Webeditor zusammenklicken; dadurch können Codex-Snapshot, PR-Branch und echter `main` auseinanderlaufen.
